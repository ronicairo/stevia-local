window.addEventListener('DOMContentLoaded', function () {
    const table = $('#creance-sans-mouv-dataTable').DataTable({
        dom: 'lt<"bottom d-flex justify-content-between align-items-center"i p>B',
        ajax: {
            url: Routing.generate('pilotage_statistique_revision_dossier_low_pss_sans_mvts_get_data', {route: datasRoute}),
            data: function (d) {
                d.filters = getFilters('creance-sans-mouv-dataTable')
                return d;
            }
        },
        order: [[2, 'asc']],
        ...
        buttons: [
            {
                ...
                action: function () {
                    $.ajax({
                        url: Routing.generate('creances_pss_sans_mvt'),
                        data: JSON.stringify(
                            {
                                filters: getFilters('creance-sans-mouv-dataTable'),
                                route: datasRoute,
                            }
                        ),
                        method: "POST",
                        success: (response) => {
                            ...
                            if (response.startsWith('Numero reference')) {
                                ...
                                link.download = 'creances_sans_mouv.csv';
                                link.click();
    ...
    const footers = {
        totalMontantInitial: $('#creance-sans-mouv-dataTable_wrapper .dt-scroll-foot tfoot tr th:eq(7)'),
        totalPartMutuelle: $('#creance-sans-mouv-dataTable_wrapper .dt-scroll-foot tfoot tr th:eq(8)'),
        totalSolde: $('#creance-sans-mouv-dataTable_wrapper .dt-scroll-foot tfoot tr th:eq(9)')
    }
    const route = Routing.generate('supervision_creances_liste_soldes')

    initializeSoldes(table, footers, route, {route: datasRoute});
});