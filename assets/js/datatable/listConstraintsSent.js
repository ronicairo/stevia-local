window.addEventListener('DOMContentLoaded', function () {
    const table = $('#list-constraints-sent').DataTable({
        dom: 'lt<"bottom d-flex justify-content-between align-items-center"i p>B',
        ajax: {
            url: Routing.generate('supervision_contrainte_envoyee_get_data'),
            data: function (d) {
                d.filters = getFilters('list-constraints-sent')
                return d;
            }
        },
        columns: [
        ...
        buttons: [
            {
                ...
                action: function () {
                    $.ajax({
                        url: Routing.generate('supervision_contrainte_liste_export'),
                        data: JSON.stringify({
                                filters: getFilters('list-constraints-sent')
                            }
                        ),
                        method: "POST",
                        success: (response) => {
                            ...
                            link.download = 'liste_contraintes_envoyees.csv';
                            link.click();
                        ...
                        error: function () {
                            ...
                            $('#list-constraints-sent').DataTable().processing(false);
